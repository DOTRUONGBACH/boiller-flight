...
// The TicketOwnerFunc type is an adapter to allow the use of ordinary
// function as TicketOwner mutator.
type TicketOwnerFunc func(context.Context, *ent.TicketOwnerMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TicketOwnerFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.TicketOwnerMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TicketOwnerMutation", m)
	}
	return f(ctx, mv)
}
