...
type TicketQuery struct {
	...
	withFlightOwner  *FlightQuery
	withBookingOwner *BookingQuery
	withTicketOwner  *TicketOwnerQuery
	withFKs          bool
	modifiers        []func(*sql.Selector)
	loadTotal        []func(context.Context, []*Ticket) error
...
// QueryFlightOwner chains the current query on the "flight_owner" edge.
func (tq *TicketQuery) QueryFlightOwner() *FlightQuery {
	query := &FlightQuery{config: tq.config}
...
// QueryBookingOwner chains the current query on the "booking_owner" edge.
func (tq *TicketQuery) QueryBookingOwner() *BookingQuery {
	query := &BookingQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryTicketOwner chains the current query on the "ticket_owner" edge.
func (tq *TicketQuery) QueryTicketOwner() *TicketOwnerQuery {
	query := &TicketOwnerQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TicketQuery) Clone() *TicketQuery {
	...
	return &TicketQuery{
		...
		withFlightOwner:  tq.withFlightOwner.Clone(),
		withBookingOwner: tq.withBookingOwner.Clone(),
		withTicketOwner:  tq.withTicketOwner.Clone(),
...
// WithFlightOwner tells the query-builder to eager-load the nodes that are connected to
// the "flight_owner" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TicketQuery) WithFlightOwner(opts ...func(*FlightQuery)) *TicketQuery {
	query := &FlightQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withFlightOwner = query
	return tq
}

// WithBookingOwner tells the query-builder to eager-load the nodes that are connected to
// the "booking_owner" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TicketQuery) WithBookingOwner(opts ...func(*BookingQuery)) *TicketQuery {
	query := &BookingQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withBookingOwner = query
	return tq
}

// WithTicketOwner tells the query-builder to eager-load the nodes that are connected to
// the "ticket_owner" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TicketQuery) WithTicketOwner(opts ...func(*TicketOwnerQuery)) *TicketQuery {
	query := &TicketOwnerQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withTicketOwner = query
	return tq
}
...
func (tq *TicketQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ticket, error) {
	var (
		...
		_spec       = tq.querySpec()
		loadedTypes = [3]bool{
			tq.withFlightOwner != nil,
			tq.withBookingOwner != nil,
			tq.withTicketOwner != nil,
		}
	)
	if tq.withFlightOwner != nil || tq.withBookingOwner != nil || tq.withTicketOwner != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withFlightOwner; query != nil {
		if err := tq.loadFlightOwner(ctx, query, nodes, nil,
			func(n *Ticket, e *Flight) { n.Edges.FlightOwner = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withBookingOwner; query != nil {
		if err := tq.loadBookingOwner(ctx, query, nodes, nil,
			func(n *Ticket, e *Booking) { n.Edges.BookingOwner = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withTicketOwner; query != nil {
		if err := tq.loadTicketOwner(ctx, query, nodes, nil,
			func(n *Ticket, e *TicketOwner) { n.Edges.TicketOwner = e }); err != nil {
			return nil, err
		}
	}
...
func (tq *TicketQuery) loadFlightOwner(ctx context.Context, query *FlightQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Flight)) error {
...
func (tq *TicketQuery) loadBookingOwner(ctx context.Context, query *BookingQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Booking)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Ticket)
	for i := range nodes {
...
func (tq *TicketQuery) loadTicketOwner(ctx context.Context, query *TicketOwnerQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *TicketOwner)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Ticket)
	for i := range nodes {
		if nodes[i].ticket_owner_ticket == nil {
			continue
		}
		fk := *nodes[i].ticket_owner_ticket
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	}
	query.Where(ticketowner.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "ticket_owner_ticket" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
