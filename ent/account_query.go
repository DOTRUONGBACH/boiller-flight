...
// QueryAccOwner chains the current query on the "acc_owner" edge.
func (aq *AccountQuery) QueryAccOwner() *CustomerQuery {
	query := &CustomerQuery{config: aq.config}
...
func (aq *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withAccOwner: aq.withAccOwner.Clone(),
...
// WithAccOwner tells the query-builder to eager-load the nodes that are connected to
// the "acc_owner" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithAccOwner(opts ...func(*CustomerQuery)) *AccountQuery {
	query := &CustomerQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withAccOwner = query
	return aq
}
...
func (aq *AccountQuery) loadAccOwner(ctx context.Context, query *CustomerQuery, nodes []*Account, init func(*Account), assign func(*Account, *Customer)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "customer_accounts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
