...
func (cq *CustomerQuery) QueryTicket() *TicketOwnerQuery {
	query := &TicketOwnerQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CustomerQuery) Clone() *CustomerQuery {
	...
	return &CustomerQuery{
		...
		withTicket:   cq.withTicket.Clone(),
...
func (cq *CustomerQuery) WithTicket(opts ...func(*TicketOwnerQuery)) *CustomerQuery {
	query := &TicketOwnerQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Customer, error) {
	...
	if query := cq.withTicket; query != nil {
		if err := cq.loadTicket(ctx, query, nodes, nil,
			func(n *Customer, e *TicketOwner) { n.Edges.Ticket = e }); err != nil {
			return nil, err
		}
	}
...
func (cq *CustomerQuery) loadTicket(ctx context.Context, query *TicketOwnerQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *TicketOwner)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Customer)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.TicketOwner(func(s *sql.Selector) {
		s.Where(sql.InValues(customer.TicketColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.customer_ticket
		if fk == nil {
			return fmt.Errorf(`foreign-key "customer_ticket" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "customer_ticket" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
