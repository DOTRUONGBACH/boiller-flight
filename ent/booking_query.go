...
type BookingQuery struct {
	...
	withBookingFlight           *FlightQuery
	withCustomerBookingTickets  *CustomerQuery
	withPassengerBookingTickets *PassengerQuery
	withBookingTickets          *TicketQuery
	modifiers                   []func(*sql.Selector)
	loadTotal                   []func(context.Context, []*Booking) error
	withNamedBookingTickets     map[string]*TicketQuery
...
// QueryBookingFlight chains the current query on the "booking_flight" edge.
func (bq *BookingQuery) QueryBookingFlight() *FlightQuery {
	query := &FlightQuery{config: bq.config}
...
// QueryCustomerBookingTickets chains the current query on the "customer_booking_tickets" edge.
func (bq *BookingQuery) QueryCustomerBookingTickets() *CustomerQuery {
...
// QueryPassengerBookingTickets chains the current query on the "passenger_booking_tickets" edge.
func (bq *BookingQuery) QueryPassengerBookingTickets() *PassengerQuery {
	query := &PassengerQuery{config: bq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := bq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryBookingTickets chains the current query on the "booking_tickets" edge.
func (bq *BookingQuery) QueryBookingTickets() *TicketQuery {
	query := &TicketQuery{config: bq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := bq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (bq *BookingQuery) IDs(ctx context.Context) ([]uuid.UUID, error) {
	var ids []uuid.UUID
...
func (bq *BookingQuery) Clone() *BookingQuery {
	...
	return &BookingQuery{
		...
		withBookingFlight:           bq.withBookingFlight.Clone(),
		withCustomerBookingTickets:  bq.withCustomerBookingTickets.Clone(),
		withPassengerBookingTickets: bq.withPassengerBookingTickets.Clone(),
		withBookingTickets:          bq.withBookingTickets.Clone(),
...
// WithBookingFlight tells the query-builder to eager-load the nodes that are connected to
// the "booking_flight" edge. The optional arguments are used to configure the query builder of the edge.
func (bq *BookingQuery) WithBookingFlight(opts ...func(*FlightQuery)) *BookingQuery {
	query := &FlightQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	bq.withBookingFlight = query
	return bq
}

// WithCustomerBookingTickets tells the query-builder to eager-load the nodes that are connected to
// the "customer_booking_tickets" edge. The optional arguments are used to configure the query builder of the edge.
func (bq *BookingQuery) WithCustomerBookingTickets(opts ...func(*CustomerQuery)) *BookingQuery {
	query := &CustomerQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	bq.withCustomerBookingTickets = query
	return bq
}

// WithPassengerBookingTickets tells the query-builder to eager-load the nodes that are connected to
// the "passenger_booking_tickets" edge. The optional arguments are used to configure the query builder of the edge.
func (bq *BookingQuery) WithPassengerBookingTickets(opts ...func(*PassengerQuery)) *BookingQuery {
	query := &PassengerQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	bq.withPassengerBookingTickets = query
	return bq
}

// WithBookingTickets tells the query-builder to eager-load the nodes that are connected to
// the "booking_tickets" edge. The optional arguments are used to configure the query builder of the edge.
func (bq *BookingQuery) WithBookingTickets(opts ...func(*TicketQuery)) *BookingQuery {
	query := &TicketQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	bq.withBookingTickets = query
	return bq
}
...
func (bq *BookingQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Booking, error) {
	var (
		...
		_spec       = bq.querySpec()
		loadedTypes = [4]bool{
			bq.withBookingFlight != nil,
			bq.withCustomerBookingTickets != nil,
			bq.withPassengerBookingTickets != nil,
			bq.withBookingTickets != nil,
		}
	)
	...
	if query := bq.withBookingFlight; query != nil {
		if err := bq.loadBookingFlight(ctx, query, nodes, nil,
			func(n *Booking, e *Flight) { n.Edges.BookingFlight = e }); err != nil {
			return nil, err
		}
	}
	if query := bq.withCustomerBookingTickets; query != nil {
		if err := bq.loadCustomerBookingTickets(ctx, query, nodes, nil,
			func(n *Booking, e *Customer) { n.Edges.CustomerBookingTickets = e }); err != nil {
			return nil, err
		}
	}
	if query := bq.withPassengerBookingTickets; query != nil {
		if err := bq.loadPassengerBookingTickets(ctx, query, nodes, nil,
			func(n *Booking, e *Passenger) { n.Edges.PassengerBookingTickets = e }); err != nil {
			return nil, err
		}
	}
	if query := bq.withBookingTickets; query != nil {
		if err := bq.loadBookingTickets(ctx, query, nodes,
			func(n *Booking) { n.Edges.BookingTickets = []*Ticket{} },
			func(n *Booking, e *Ticket) { n.Edges.BookingTickets = append(n.Edges.BookingTickets, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range bq.withNamedBookingTickets {
		if err := bq.loadBookingTickets(ctx, query, nodes,
			func(n *Booking) { n.appendNamedBookingTickets(name) },
			func(n *Booking, e *Ticket) { n.appendNamedBookingTickets(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (bq *BookingQuery) loadBookingFlight(ctx context.Context, query *FlightQuery, nodes []*Booking, init func(*Booking), assign func(*Booking, *Flight)) error {
...
func (bq *BookingQuery) loadCustomerBookingTickets(ctx context.Context, query *CustomerQuery, nodes []*Booking, init func(*Booking), assign func(*Booking, *Customer)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Booking)
	for i := range nodes {
...
func (bq *BookingQuery) loadPassengerBookingTickets(ctx context.Context, query *PassengerQuery, nodes []*Booking, init func(*Booking), assign func(*Booking, *Passenger)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Booking)
	for i := range nodes {
		fk := nodes[i].PassengerID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "passenger_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (bq *BookingQuery) loadBookingTickets(ctx context.Context, query *TicketQuery, nodes []*Booking, init func(*Booking), assign func(*Booking, *Ticket)) error {
	...
	query.Where(predicate.Ticket(func(s *sql.Selector) {
		s.Where(sql.InValues(booking.BookingTicketsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "booking_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
// WithNamedBookingTickets tells the query-builder to eager-load the nodes that are connected to the "booking_tickets"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (bq *BookingQuery) WithNamedBookingTickets(name string, opts ...func(*TicketQuery)) *BookingQuery {
	query := &TicketQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	if bq.withNamedBookingTickets == nil {
		bq.withNamedBookingTickets = make(map[string]*TicketQuery)
	}
	bq.withNamedBookingTickets[name] = query
	return bq
}
